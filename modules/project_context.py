...
SYSTEM_CONFIG_RELATIVE_PATH = Path("user_config") / "system-config.yml"
...
_DEFAULT_PATHS: dict[str, str] = {
    ...
    "text_encoders_dir": "models/StableDiffusion/TextEncoders",
    "control_lora_dir": "models/StableDiffusion/control-lora",
    "upscalers_comfyui_dir": "models/StableDiffusion/Upscalers_ComfyUI",
...
_DEFAULT_ASSET_HUB: dict[str, Any] = {
    "downloads": {
        "max_active_downloads": 2,
        "max_queued_downloads": 64,
        "bandwidth_limit_mib_per_second": 0.0,
        "provider_min_request_interval_seconds": 0.25,
        "retry_attempts": 3,
    },
}

_DEFAULT_MANAGED_ROOTS: dict[str, str] = {
    "CheckPoints": "models/StableDiffusion/CheckPoints",
    "VAE": "models/StableDiffusion/VAE",
    "VAE_approx": "models/StableDiffusion/VAE_approx",
    "Lora": "models/StableDiffusion/Lora",
    "BLIP": "models/StableDiffusion/BLIP",
    "Codeformer": "models/StableDiffusion/Codeformer",
    "ESRGAN": "models/StableDiffusion/ESRGAN",
    "GFPGAN": "models/StableDiffusion/GFPGAN",
    "RealESRGAN": "models/StableDiffusion/RealESRGAN",
    "ControlNet": "models/StableDiffusion/ControlNet",
    "ControlLora": "models/StableDiffusion/control-lora",
    "Embeddings": "models/StableDiffusion/Embeddings",
    "Hypernetworks": "models/StableDiffusion/Hypernetworks",
    "TextEncoders": "models/StableDiffusion/TextEncoders",
    "Upscalers_ComfyUI": "models/StableDiffusion/Upscalers_ComfyUI",
}
...
def _deep_merge(base: Mapping[str, Any], override: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key, value in override.items():
        ...
        if isinstance(existing, Mapping) and isinstance(value, Mapping):
            result[skey] = _deep_merge(existing, value)
        elif isinstance(value, Mapping):
            result[skey] = _deep_copy_mapping(value)
        elif isinstance(value, list):
            result[skey] = list(value)
        else:
            result[skey] = value
    return result


def _effective_config(user_config: Mapping[str, Any]) -> dict[str, Any]:
    defaults: dict[str, Any] = {
        "project": {"config_version": 1},
        "paths": dict(_DEFAULT_PATHS),
        "model_library": {
            "managed_roots": dict(_DEFAULT_MANAGED_ROOTS),
            "additional_scan_roots": [],
        },
        "asset_hub": _deep_copy_mapping(_DEFAULT_ASSET_HUB),
    }
    effective = _deep_merge(defaults, user_config)
    sources: dict[str, str] = {}
    user_paths = user_config.get("paths") if isinstance(user_config.get("paths"), Mapping) else {}
    for key in _DEFAULT_PATHS:
        sources[f"paths.{key}"] = "user_config" if key in user_paths else "system_default"
    user_library = user_config.get("model_library") if isinstance(user_config.get("model_library"), Mapping) else {}
    user_roots = user_library.get("managed_roots") if isinstance(user_library.get("managed_roots"), Mapping) else {}
    for key in _DEFAULT_MANAGED_ROOTS:
        sources[f"model_library.managed_roots.{key}"] = "user_config" if key in user_roots else "system_default"
    user_asset_hub = user_config.get("asset_hub") if isinstance(user_config.get("asset_hub"), Mapping) else {}
    user_downloads = user_asset_hub.get("downloads") if isinstance(user_asset_hub.get("downloads"), Mapping) else {}
    for key in _DEFAULT_ASSET_HUB["downloads"]:
        sources[f"asset_hub.downloads.{key}"] = "user_config" if key in user_downloads else "system_default"
    effective["_generated"] = {
        "authority": "resolved_runtime_configuration",
        "user_override_source": CANONICAL_CONFIG_RELATIVE_PATH.as_posix(),
        "generated_path": SYSTEM_CONFIG_RELATIVE_PATH.as_posix(),
        "do_not_edit": True,
        "secrets_included": False,
    }
    effective["_config_sources"] = sources
    return effective


def _write_system_config(path: Path, payload: Mapping[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(f".{path.name}.tmp")
    text = "# GENERATED BY IMAGE_GEN AT STARTUP. DO NOT EDIT.\n" + yaml.safe_dump(
...
class ProjectContext:
    ...
    control_lora_dir: Path
    upscalers_comfyui_dir: Path
    ...
    def load(
        ...
        *,
        ...
        user_config = _deep_copy_mapping(loaded)
        config = _effective_config(user_config)
        system_config_path = (root / SYSTEM_CONFIG_RELATIVE_PATH).resolve()
        try:
            _write_system_config(system_config_path, config)
        except OSError as exc:
            raise ProjectConfigurationError(f"Unable to write generated system configuration {system_config_path}: {exc}") from exc
        ...
        return cls(
            ...
            text_encoders_dir=resolved["text_encoders_dir"],
            control_lora_dir=resolved["control_lora_dir"],
            upscalers_comfyui_dir=resolved["upscalers_comfyui_dir"],
