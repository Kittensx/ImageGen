...
valid_decay_patterns = [
    ...
    'extrapolate', 'fractional', 'soft_landing', 'exponential', 'linear'
]
valid_decay_modes = [
    'append',
    'blend',
...
def _soft_landing_factor(progress):
    progress = min(max(float(progress), 0.0), 1.0)
    smooth = progress * progress * (3.0 - 2.0 * progress)
    return max(1.0 - smooth, 0.0)
...
def apply_last_tail(sigmas, device, decay_pattern='zero'):
    ...
    elif decay_pattern == 'soft_landing':
        next_sigma = max(last_sigma * _soft_landing_factor(0.5), 1e-5)
...
def apply_decay_tail(sigmas, device, decay_pattern='geometric', tail_steps=5):
    ...
    for step in range(tail_steps):
        if decay_pattern == 'geometric':
            ...
            progress = (step + 1) / max(tail_steps, 1)
            next_sigma = max(sigmas[-1] * _soft_landing_factor(progress), 1e-5)
...
def blend_decay_tail(sigmas, device, decay_pattern='geometric', tail_steps=5):
    ...
    for i in range(1, tail_steps + 1):
        ...
        if decay_pattern == 'geometric':
            ...
            progress = (tail_steps - i + 1) / max(tail_steps, 1)
            new_sigma = max(base_sigma * _soft_landing_factor(progress), 1e-5)
