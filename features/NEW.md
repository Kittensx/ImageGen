...
## Replayable and Inspectable Structured Prompt Semantics

The reconstructed Classic prompt semantics are now versioned and replayable. New generations record PromptIR, the conditioning plan, semantic/structure digests, parser/compiler contracts, model-family semantic state, and safe-fallback diagnostics. Exact replay validates this semantic record instead of re-guessing punctuation.

Prompt Inspector includes a Semantic Structure view with group percentages, owner/relation scope, schedules, fallbacks, encoder-visible text, and effective-final weights for static prompts. The parser-only `run.bat parser-test` gate includes readable replay/cutover evidence, while a separate opt-in PPSR-08 real-checkpoint runner creates image/request/log evidence and a contact sheet for final image-level qualification.

- PPSR-08 real-checkpoint qualification now exposes live end-to-end console progress. Sampling step progress is enabled, the real `run.bat` stream is tee'd to CMD plus evidence logs, verbose model-loading diagnostics are visible, and qualification lifecycle markers show discovery, model/family transitions, request execution, image output, semantic replay, and process resource release.

## PPSR-09 experimental grouping and binding

Prompt-parser experimentation can now compare the established `{...}` group against a white-brace cohesive-group algorithm without changing the production control. PPSR-09 also adds `^` target-only binding and `*` subtree/inheriting binding with explicit local inheritance barriers. A multi-seed real-runner qualification matrix is included so these semantics can be judged by attribute binding, color leakage, and diversity rather than parser math alone.

## Corrected Legacy BREAK Semantics

Legacy Default now recognizes uppercase `BREAK` as the same typed encoder chunk-boundary operation used by the qualified PPSR-09E runtime. This includes structured SDXL and SD3/SD3.5 conditioning support while leaving Legacy `AND`, grouping, and other parser behavior unchanged. The Legacy profile is versioned forward so older replay/profile snapshots remain distinguishable.
